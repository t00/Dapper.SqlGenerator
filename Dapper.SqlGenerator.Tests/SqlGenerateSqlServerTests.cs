...
namespace Dapper.SqlGenerator.Tests
{
    ...
    public class SqlGenerateSqlServerTests
    {
        ...
        public void TestInsertReturn()
        {
            ...
            Assert.AreEqual("INSERT INTO [TestProducts] ([Type],[Content],[Enum],[MaybeDate],[Date],[MaybeGuid],[Guid],[Duration],[Last]) OUTPUT INSERTED.[id] VALUES (@Kind,@Content,@Enum,@MaybeDate,@Date,@MaybeGuid,@Guid,@Duration,@Last)", cols);
        }
        ...
        public void TestInsertKeysReturn()
        {
            ...
            Assert.AreEqual("INSERT INTO [TestProducts] ([id],[Type],[Content],[Enum],[MaybeDate],[Date],[MaybeGuid],[Guid],[Duration],[Last]) OUTPUT INSERTED.[id] VALUES (@Id,@Kind,@Content,@Enum,@MaybeDate,@Date,@MaybeGuid,@Guid,@Duration,@Last)", cols);
        }
